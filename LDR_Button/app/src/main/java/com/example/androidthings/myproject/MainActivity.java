...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            // set LDR as button for LED
            // Create GPIO connection.
            mLdrGpio = service.openGpio(LDR_PIN);
            // Configure as an input.
            mLdrGpio.setDirection(Gpio.DIRECTION_IN);
            // Enable edge trigger events for both falling and rising edges. This will make it a toggle button.
            mLdrGpio.setEdgeTriggerType(Gpio.EDGE_BOTH);
            // Register an event callback.
            mLdrGpio.registerGpioCallback(mSetLEDCallback);
    ...
    protected void onDestroy() {
        ...
        // Close the resource
        if (mLdrGpio != null) {
            mLdrGpio.unregisterGpioCallback(mSetLEDCallback);
            try {
                mLdrGpio.close();
            } catch (IOException e) {
                Log.e(TAG, "Error on PeripheralIO API", e);
